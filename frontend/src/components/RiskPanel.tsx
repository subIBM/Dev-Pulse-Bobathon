...
export const RiskPanel: React.FC<RiskPanelProps> = ({
  ...
  return (
    <div className="glass-panel-strong overflow-hidden p-6">
      <div key={riskScore.file} className="risk-panel-content space-y-6">
        <div className="border-b border-white/10 pb-5">
          ...
          <div className="flex items-start justify-between gap-4">
            ...
            <span className={`${getRiskBadgeClass(riskScore.risk_level)} risk-badge-animate`}>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="metric-tile risk-metric-card bg-cyan-400/8">
          ...
          <div className="metric-tile risk-metric-card bg-violet-400/8">
            <div className="mb-3 flex items-center gap-2 text-sm font-medium text-slate-300">
        ...
        <div className="risk-info-card rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4 backdrop-blur-xl">
        ...
        {riskScore.predicted_conflict_date && (
          <div className="risk-info-card rounded-2xl border border-rose-400/20 bg-rose-400/10 p-4 backdrop-blur-xl">
        ...
        <div>
          ...
          <ul className="space-y-3">
            {riskScore.reasons.map((reason, index) => (
              <li
                key={`${riskScore.file}-${index}-${reason}`}
                className="risk-reason-item rounded-2xl border border-white/10 bg-white/6 px-4 py-3 text-sm text-slate-200 backdrop-blur-xl"
        ...
        <div className="border-t border-white/10 pt-4">
          <button onClick={onRefactorClick} disabled={isRefactoring} className="btn-primary w-full gap-2">
            {isRefactoring ? (
              <>
                ...
                <span>ICA Agent Analyzing...</span>
              </>
            ) : (
              <>
                <Sparkles className="h-4 w-4" />
                <span>Generate ICA Agent Refactor</span>
              </>
            )}
          </button>
          ...
          <p className="mt-3 text-center text-xs text-slate-400">
            ICA Agent will generate a safer structure to reduce complexity and refactor risk.
          </p>
        </div>
      </div>
    </div>
